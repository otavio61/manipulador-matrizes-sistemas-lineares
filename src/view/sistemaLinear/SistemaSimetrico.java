...
public class SistemaSimetrico extends JPanel{
    ...
    private void novoSistema(){
        ...
        JTextField[][] vetor = new JTextField[ordem][ordem];
        ...
        for (int i = 0; i < vetor.length; i++) {
            for (int j = i; j < vetor.length; j++) {
                ...
                if(i != j){
                    vetor[i][j].getDocument().addDocumentListener(new DocumentListener() {

                        @Override
                        public void insertUpdate(DocumentEvent e) {
                            vetor[j1][i1].setText(vetor[i1][j1].getText());
                        }
    ...
    private void resolver(){
        ...
        if(!Matrizes.criterioSylvester(Matrizes.removerColuna(sistema, sistema[0].length))){
            JOptionPane.showMessageDialog(this, "A matriz dos coeficientes não é definida positiva", "Sistema simétrico", JOptionPane.INFORMATION_MESSAGE);
            return;
        }

        double[][] matrizL = SistemasLineares.fatoracaoCholesky(sistema);

        double[][] resultado = SistemasLineares.substituicaoRetroativa(Matrizes.adicionarColuna(Matrizes.transposicao(Matrizes.removerColuna(matrizL, matrizL[0].length)), SistemasLineares.substituicaoSucessiva(matrizL)));
