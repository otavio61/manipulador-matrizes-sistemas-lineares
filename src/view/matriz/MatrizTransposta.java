...
public class MatrizTransposta extends JPanel{
    ...
    private JButton bt_novaMatriz, bt_limpar, bt_transposicao, bt_preencher;
    ...
    public MatrizTransposta(){
        ...
        bt_transposicao = new JButton("Transpor");
        bt_transposicao.addActionListener(e -> {transposicao();});
        bt_transposicao.setMnemonic('m');
    ...
    private void novaMatriz(){
        ...
        try{
            if(linha.isBlank() || Integer.parseInt(linha) < 0 || coluna.isBlank() || Integer.parseInt(coluna) < 0){
                JOptionPane.showMessageDialog(this, "Selecione valores maiores que 0 para determinar o número de linhas e colunas da matriz", "Atenção!", JOptionPane.WARNING_MESSAGE);
                return;
            }
    ...
    private void transposicao(){
        ...
        double[][] matrizTransposta = Matrizes.transposicao(matriz);

        String msg = Matrizes.formatoLaTeX(matriz) + "^T =" + Matrizes.formatoLaTeX(matrizTransposta);
        ...
        JOptionPane.showMessageDialog(this, panel, "Resultado da transposição", JOptionPane.PLAIN_MESSAGE);
    }
